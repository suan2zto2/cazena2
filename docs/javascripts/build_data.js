...
async function buildAndDownloadData() {
  withBtn('btn-build-data', '🔄 구글 시트에서 JSON 만들기 (전체)', async (set) => {
    await loadJSZip();
    set('⏳ 시트 읽는 중...');
    const [stageRows, smRows, charRows, cardRows, monRows, actRows, bpRows] = await Promise.all([
      fetchSheet('Stage'), fetchSheet('StageMonster'), fetchSheet('Character'),
      fetchSheet('Card'), fetchSheet('Monster'), fetchSheet('MonsterAction'), fetchSheet('BossPhase'),
    ]);
